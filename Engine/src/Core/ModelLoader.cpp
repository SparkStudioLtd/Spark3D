...
GPUMesh* ModelLoader::loadMeshFromAsset(GPUContext* context, Asset* asset)
{

	Assimp::Importer importer;
	//const aiScene* scene = importer.ReadFile(file.c_str(), aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices);
	const aiScene* scene = importer.ReadFileFromMemory(&asset->m_LoadedAsset.blob[0], asset->m_LoadedAsset.blob.size(), aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_JoinIdenticalVertices);
